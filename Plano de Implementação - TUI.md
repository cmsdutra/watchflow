...
**Implementado** (Linux, validado contra daemon real):

| Item | Situação |
|---|---|
| `internal/tui/` — Model, Update, View (Elm Architecture) | ✅ |
| `cmd/watchflow/tui.go` — subcomando Cobra `watchflow tui` | ✅ |
| Painel de cofres vigiados com navegação e seleção | ✅ |
| Painel da fila persistente SQLite | ✅ |
| Painel de auditoria e eventos recentes | ✅ |
| Atalhos `s` / `p` / `r` / `Tab` / `↑↓` / `q` | ✅ |
| Tela de daemon offline com reconexão automática | ✅ |
| Testes determinísticos sem TTY (`internal/tui/tui_test.go`) | ✅ |

### 6.1. Decisão: origem dos dados de auditoria

O mockup da seção 3 mostra **uma linha por step** (`git.add`, `git.commit`, `git.safe_sync`).
A tabela `pipeline_runs` guarda **uma linha por execução**, não por step.

A auditoria é lida do **log JSON estruturado** (`<state_dir>/watchflow.log`), via
`logger.Tail`, em vez de uma nova tabela `pipeline_steps`. Justificativa:

* O log já registra cada step com componente, duração e resultado — a granularidade
  do mockup sai sem mudança de schema.
* Evita duplicar a mesma informação em dois lugares e manter duas políticas de retenção.
* Menos escritas por job no caminho quente do pipeline.

**Consequência:** a TUI depende do arquivo de log **além** do socket. Se o log estiver
ausente ou ilegível, os demais painéis seguem funcionando e apenas o de auditoria fica
vazio — a degradação é parcial e silenciosa por decisão de projeto.

---

## 7. Limitação Conhecida: Portabilidade (macOS / Windows)

> **Status:** ⚠️ Não implementado. Pendente de trabalho futuro com testes adequados.

O objetivo 4 da seção 1 previa compilação estática multiplataforma. **A implementação
atual foi desenvolvida e validada exclusivamente em Linux.**

### 7.1. O que já é portável

`bubbletea` e `lipgloss` são pure-Go e multiplataforma; nada na camada de interface
introduz dependência de plataforma. O binário continua compilando com `CGO_ENABLED=0`.

### 7.2. O acoplamento real está no daemon, não na TUI

A TUI é apenas um cliente. O que impede o WatchFlow de rodar fora do Linux hoje:

| Componente | Acoplamento |
|---|---|
| `internal/watcher` | `inotify` via fsnotify (no Windows seria `ReadDirectoryChangesW`, com semântica distinta de eventos) |
| `internal/ipc` | Unix Domain Socket em `/run/user/<UID>/watchflow.sock` (no Windows exigiria Named Pipes ou TCP em loopback) |
| `deploy/systemd` | `systemd --user` (no Windows seria um Serviço ou Tarefa Agendada) |
| `cmd/watchflow/doctor.go` | Leitura de `fs.inotify.max_user_watches` via `/proc/sys` |
| `internal/notify` (backend `desktop`) | `notify-send` / libnotify |
| `cmd/watchflow/tui.go` | Detecção de TTY por `os.ModeCharDevice` — funciona no Windows, mas o comportamento de console não foi verificado |

### 7.3. Trabalho necessário para portar

1. Abstrair o transporte IPC atrás de uma interface, com implementações por
   plataforma (`ipc_unix.go` / `ipc_windows.go` via build tags).
2. Validar o comportamento do fsnotify no Windows, em especial a criação recursiva
   de diretórios e a corrida tratada por `emitExistingEntries`.
3. Backend de notificação por plataforma (`toast` no Windows, `osascript` no macOS).
4. Adaptar `doctor` para não pressupor `/proc`.
5. **Testes de integração executando em runners Windows e macOS** — a validação
   por compilação cruzada não é suficiente, dada a diferença semântica dos eventos
   de sistema de arquivos entre plataformas.

### 7.4. Verificação mínima recomendada no interim

Adicionar ao CI, para detectar quebras de compilação cruzada antes que se acumulem:
...
Isso valida apenas a compilação, **não** o comportamento em runtime.
